...
# Create new config
curl -fsSL ${STATIC_URL}/php-fpm/pool_sub.conf -o pool_sub.conf

sed -i "s!||FPM_PM||!${FPM_PM-ondemand}!g" pool_sub.conf
sed -i "s!||FPM_MAX_CHILDREN||!${FPM_MAX_CHILDREN-5}!g" pool_sub.conf
sed -i "s!||FPM_MAX_REQUESTS||!${FPM_MAX_REQUESTS-300}!g" pool_sub.conf
...
sed -i "s!||PHP_MAX_MEMORY_LIMIT||!${PHP_MAX_MEMORY_LIMIT-256}!g" pool_sub.conf
