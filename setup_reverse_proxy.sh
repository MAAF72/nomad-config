...
curl --silent --remote-name https://raw.githubusercontent.com/MAAF72/nomad-config/php-fpm-config/reverse_proxy
...
if test -f "/etc/nginx/sites-enabled/$DEFAULT_CFG_NAME"; then
    sudo rm -f /etc/nginx/sites-enabled/$DEFAULT_CFG_NAME
fi

if test -f "/etc/nginx/sites-available/$DEFAULT_CFG_NAME"; then
    sudo rm -f /etc/nginx/sites-available/$DEFAULT_CFG_NAME
fi
